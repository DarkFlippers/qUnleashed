...
commits exactly one came from an outside contributor. Its suggestion — that a
`CONTRIBUTING.md` would be a cheaper lever than any of the five migrations —
was overtaken while this review was being written: one arrived in #127.
...
**Outside the table, as a result of the review:** the barrier to entry for an
outside contributor — one such commit out of ~850 — is a cheaper thing to
improve than any migration here. A `CONTRIBUTING.md` landed in #127 after this
was written.
...
Outside the table: one outside commit out of ~850. The `CONTRIBUTING.md` this
review asked for arrived independently in #127.
