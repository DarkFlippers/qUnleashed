...
> What the code on `main` (`35dd0f5`) does, with no verdicts and no
...
> pins, `81a4a8c` — that submodule moves independently, and its `dev` branch
...
`lib/` holds 458 `.dart` files:

| Directory | Files | Contents |
|---|---|---|
| `lib/pages/` | 232 | 7 features: `about`, `apps`, `archive`, `devices`, `flibler`, `option`, `tools` |
| `lib/modules/` | 130 | 3 local packages (see §12) |
| `lib/components/` | 44 | shared widgets and utilities |
| `lib/services/` | 41 | cross-cutting services |
| `lib/theme/` | 5 | theming |
...
1. **Hand-rolled singletons** — 30 declarations of the form
...
1. **A hand-rolled registry** for cross-feature navigation.
   ...
   (`navigation.dart:56`).
2. **Direct `Navigator`** inside a feature: 16 `Navigator.of(...).push`
   calls, 25 `MaterialPageRoute`.
...
| `} catch (` | 375 |
| `} on <Type> catch` | 23 |
| `catch (_) {}` (empty) | 50 — 43 in the app, 7 in `flipperlib` (4 of those inside an isolate) |
| `implements Exception` (own types) | 27 |

**Mixed:** 375 untyped `catch` against 23 typed ones.
...
- **The submodules are covered differently.** Counting all the code owned by
  ...
  |---|---|---|
  | app (`lib/` without `modules/`) | 328 | 51 |
  | `flipperlib` | 96 | **0** |
  | `dartufbt` | 31 | 1 |
...
└───────────────┬─────────────────────────────────────────────┘
                ...
                ▼
┌─────────────────────────────────────────────────────────────┐
│ Logic           ChangeNotifier inside features (31 classes) │
│                 + singletons in lib/services/ (30 instance) │
└───────────────┬─────────────────────────────────────────────┘
