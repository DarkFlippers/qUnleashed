...
class ArchiveKeyMeta {
  const ArchiveKeyMeta({this.protocol, this.extra, this.meta = const {}});
...
ArchiveKeyMeta _parseNfc(List<String> lines) {
  ...
  final mifareType =
      _field(lines, 'Mifare Classic type') ??
  ...
  return ArchiveKeyMeta(
    protocol: typeLabel,
    extra: uid,
    meta: {'device_type': ?deviceType, 'uid': ?uid, 'mifare_type': ?mifareType},
  );
}
...
ArchiveKeyMeta _parseRfid(List<String> lines) {
  ...
  return ArchiveKeyMeta(
    protocol: keyType,
    extra: data,
    meta: {'key_type': ?keyType, 'data': ?data, 'bit': ?bit},
  );
}
...
ArchiveKeyMeta _parseBadUsb(List<String> lines) {
  ...
  return ArchiveKeyMeta(
    ...
    extra: comment,
    meta: {'kind': 'DuckyScript', 'lines': '$lineCount', 'comment': ?comment},
  );
}
