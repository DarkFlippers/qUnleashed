    ...
    return SelectionArea(
      child: Html(
        ...
        style: {
          ...
          'li': Style(margin: Margins.only(bottom: 6), color: baseTextColor),
...
String _sanitizeHtml(String html) {
  return html
      ...
      .replaceAll(
        RegExp(
          r'\s(style|valign|vertical-align)="[^"]*"',
          caseSensitive: false,
        ),
        '',
      )
...
class _ChangelogImage extends StatelessWidget {
  const _ChangelogImage({required this.src, this.alt, this.width, this.height});
  ...
  Widget build(BuildContext context) {
    ...
    return _BaselineSafeBox(width: w, height: h, child: picture);
  }
}
...
class _BaselineSafeBox extends SingleChildRenderObjectWidget {
  ...
  void updateRenderObject(
    ...
    _RenderBaselineSafeBox renderObject,
  ) {
    renderObject
      ..fixedWidth = width
      ..fixedHeight = height;
