...
//-----------------------------------------------------------------------------
// qUnleashed adaptation (host-side offline solve for the companion app):
...
//     qunleashed_hardnested_recover(): the app-facing entry that takes parsed
//     nonces and runs the same offline solve as mfnestedhard()'s file path.
//   Everything else (the attack itself) is upstream and untouched.
//-----------------------------------------------------------------------------
#include <stdio.h>
#include <stdlib.h>
#include <inttypes.h>
...
static int acquire_nonces(uint8_t blockNo, uint8_t keyType, uint8_t *key, uint8_t trgBlockNo, uint8_t trgKeyType, bool nonce_file_write, bool slow, char *filename) {
    // comms path stripped for host-side offline solve (nonces come from Flipper .nested.log).
    (void)blockNo; (void)keyType; (void)key; (void)trgBlockNo; (void)trgKeyType;
    (void)nonce_file_write; (void)slow; (void)filename;
    return PM3_EFAILED;
}
...
// ---------------------------------------------------------------------------
// qUnleashed host-side bridge.
//
// The Flipper firmware only collects nonces (into .nested.log); the whole
// hardnested attack runs here on the companion-app host. This entry point
// takes the parsed nonces directly (no PM3 device comms, no on-disk nonce
// file) and runs the same offline solve as mfnestedhard()'s file path.
//
//   in_cuid  : 4-byte card UID ("cuid" field of the .nested.log line)
//   nt_enc   : array of `count` encrypted nonces (32-bit each)
//   par_enc  : array of `count` encrypted-parity nibbles; bit3 = parity of the
//              MSB nonce byte ... bit0 = LSB byte (PM3 add_nonce convention)
...
void qunleashed_hardnested_set_tables_path(const char *path) {
    ...
    strncpy(g_hardnested_tables_path, path, sizeof(g_hardnested_tables_path) - 1);
...
int qunleashed_hardnested_recover(uint32_t in_cuid,
    ...
    brute_force_per_second = brute_force_benchmark();
    write_stats = false;
    known_target_key = -1;  // real attack: key is unknown
    cuid = in_cuid;
    start_time = msclock();
    ...
    for (uint32_t i = 0; i < count; i++) {
        num_acquired_nonces += add_nonce(nt_enc[i], par_enc[i]);
    }

    // Map the accumulated raw first_byte_Sum to its index in sums[] (exactly as
    // read_nonce_file does). generate_candidates() expects the index, not the
    // raw sum. No match => the nonces aren't a genuine hardened MFC set, or too
    // few were supplied to cover all 256 first bytes.
    bool got_match = false;
    for (uint8_t i = 0; i < NUM_SUMS; i++) {
        if (first_byte_Sum == sums[i]) {
            first_byte_Sum = i;
    ...
    if (expected_brute_force1 < expected_brute_force2) {
        // ignoring Sum(a8) properties is cheaper: brute force the smallest bitarray
        add_bitflip_candidates(best_first_byte_smallest_bitarray);
        maximum_states = 0;
        for (statelist_t *sl = candidates; sl != NULL; sl = sl->next) {
            maximum_states += (uint64_t)sl->len[ODD_STATE] * sl->len[EVEN_STATE];
